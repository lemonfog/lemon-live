...
<template>
  <div inline-block pos-relative @click="click" cursor-pointer text-center>
    ...
    <div  v-show="show" z-1 pos-absolute top-0 class="left-50% " w-max style="transform:translate3d(-50%,-100%,0);">
      <div :data-index="index"  v-for="i, index in list" :class="{ 'text-amber': index == active }" px-2 py-1 flex items-center hover:text-amber>
        {{ i }} 
      </div>
    </div>
  </div>
